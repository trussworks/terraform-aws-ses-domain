...
func TestTerraformSESDomainWithSPFEnabled(t *testing.T) {
	...
	awsRegion := "us-west-2"
	fromAddresses := fmt.Sprintf("no-reply@%s", testDomain)
...
func TestTerraformSESDomainWithExtraSESRecords(t *testing.T) {
	...
	fromAddresstestDomain := fmt.Sprintf("%s.infra-test.truss.coffee", testName)
	sesBucketName := fmt.Sprintf("%s-ses", testName)
	awsRegion := "us-west-2"
	extraRecords := []string{"stringThing1.infra-test.truss.coffee", "stringThing2.infra-test.truss.coffee"}
	fromAddresses := fmt.Sprintf("no-reply@%s", fromAddresstestDomain)
...
func TestTerraformSESDomainWithIncomingEmailDisabled(t *testing.T) {
	t.Parallel()
